import { coffeeService } from '../services/farmService';

// Presenter - Business logic for Coffee Farm operations
class CoffeePresenter {
  ...
  async loadFarms() {
    try {
      ...
      const result = await coffeeService.getAllFarms();
  ...
  async createFarm(farmData) {
    try {
      this.view?.setLoading(true);
      const result = await coffeeService.createFarm(farmData);
  ...
  async updateFarm(farmId, farmData) {
    try {
      this.view?.setLoading(true);
      const result = await coffeeService.updateFarm(farmId, farmData);
  ...
  async deleteFarm(farmId) {
    try {
      this.view?.setLoading(true);
      const result = await coffeeService.deleteFarm(farmId);
